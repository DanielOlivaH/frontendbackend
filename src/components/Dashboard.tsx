    ...
    <Box sx={{ p: 2 }}>
      <Paper sx={{ p: 2, mb: 2 }}>
        ...
        <Box component="form" onSubmit={handleInsert} sx={{ display: 'grid', gap: 2 }}>
          ...
          <Tooltip title="Insertar nuevo elemento" arrow placement="top">
             <Button fullWidth sx={{ mt: 2 }} variant="contained" onClick={handleInsert}>INSERTAR</Button>
          </Tooltip>
        </Box>
      </Paper>
          ...
          <TableBody>
            {tableData.map(row => (
              <TableRow key={row.id}>
                <TableCell>
                  {userRol === 'admin' ? (
                    <Tooltip title="Eliminar elemento" arrow placement="left">
